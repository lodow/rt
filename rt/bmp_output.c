...
** Last update Fri May 24 15:48:43 2013 Adrien Della Maggiora
*/
...
void		output_bmp(t_par *ppt)
{
  ...
  if (ppt->bpp != 32 && ppt->bpp != 24)
    return ;
  init_bmp(&image);
  if ((fd = open("result.bmp", O_WRONLY | O_CREAT | O_TRUNC, 0664)) == -1)
    {
      my_putstr("Create File Failed.\n", 2);
      return ;
    }
